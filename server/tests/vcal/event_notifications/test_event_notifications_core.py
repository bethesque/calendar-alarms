...
from homeaudio.vcal.event_notifications.core import ERROR_MESSAGE_AUDIO, _build_notification_files, check_for_event_notifications
...
def _stub_audio_building(monkeypatch):
    ...
    # concerned with the targets logic in _build_notification_files.
...
def test_build_notification_files_uses_shared_target_when_all_notifications_agree(monkeypatch, tmp_path):
    ...
    announcements_file, alarm_audio_file = _build_notification_files(
        [announcement_notification, alarm_notification], base_time, EventNotificationSettings()
    )
...
def test_build_notification_files_returns_none_target_when_notifications_disagree(monkeypatch, tmp_path):
    _stub_audio_building(monkeypatch)
    base_time = datetime.fromisoformat("2026-04-06T08:00:00+10:00")
    ...
    announcements_file, alarm_audio_file = _build_notification_files(
        [kitchen_notification, bedroom_notification], base_time, EventNotificationSettings()
    )
...
def test_build_notification_files_returns_none_target_when_one_notification_is_untargeted(monkeypatch, tmp_path):
    _stub_audio_building(monkeypatch)
    base_time = datetime.fromisoformat("2026-04-06T08:00:00+10:00")
    ...
    announcements_file, _ = _build_notification_files(
        [kitchen_notification, untargeted_notification], base_time, EventNotificationSettings()
    )

    assert announcements_file.targets is None


def test_build_notification_files_returns_none_target_when_no_notification_has_targets(monkeypatch, tmp_path):
    _stub_audio_building(monkeypatch)
    base_time = datetime.fromisoformat("2026-04-06T08:00:00+10:00")
    ...
    announcements_file, alarm_audio_file = _build_notification_files(
        [first_notification, second_notification], base_time, EventNotificationSettings()
    )
